...
from jesFinance.forms import AccountCreateForm, ForeignAccountForm, ReconcilationForm
from jesFinance.models import Account, Split, Transaction
...
class AccountIndex(LoginRequiredMixin, generic.TemplateView):
    template_name = 'jesFinance/accounts.html'
...
class ForeignAccountIndex(LoginRequiredMixin, generic.ListView):
    template_name = 'jesFinance/foreign_accounts.html'
    queryset = Account.objects.foreign()
    paginate_by = 20


class AccountView(LoginRequiredMixin, generic.ListView):
    template_name = 'jesFinance/account_detail.html'
...
class ReconcileView(LoginRequiredMixin, generic.edit.CreateView):
    template_name = 'jesFinance/reconcile.html'
